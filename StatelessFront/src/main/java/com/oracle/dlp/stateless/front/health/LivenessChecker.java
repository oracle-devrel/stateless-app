...
package com.oracle.dlp.stateless.front.health;
...
import com.oracle.dlp.stateless.front.resources.AutoHang;
...
public class LivenessChecker implements HealthCheck {
	...
	public HealthCheckResponse call() {
		// don't test anything here, we're just reporting that we are running (or not),
		// not that
		...
		if (autoHang.autoTimerTest()) {
			log.info("Auto hang delay is active");
			autoHang.doHang();
			return HealthCheckResponse.named("greeting-service").up()
					.withData("uptime", System.currentTimeMillis() - startTime)
					.withData("hangDelay", autoHang.getHangSeconds()).build();
		}
		log.info("Auto hang delay is inactive");
		return HealthCheckResponse.named("greeting-service").up()
				.withData("uptime", System.currentTimeMillis() - startTime)
				.withData("hangDelay", autoHang.getHangSeconds()).build();
	}
